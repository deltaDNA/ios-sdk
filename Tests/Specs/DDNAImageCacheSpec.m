...
describe(@"image cache", ^{
    ...
    __block UIImage *mockImage;
    
    beforeEach(^{
        imageCache = [[DDNAImageCache alloc] init];
        mockURL = mock([NSURL class]);
        mockImage = mock([UIImage class]);
    });
    ...
    it(@"stores images", ^{
        ...
        [imageCache setImage:mockImage forURL:mockURL];
        
        UIImage *image2 = [imageCache imageForURL:mockURL];
        expect(image2).to.equal(mockImage);
    });
});
